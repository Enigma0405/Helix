#!/usr/bin/env python3
"""Project Helix — System Integration Verification Script.

Systematically validates all 13 core workflows of the MVP stack, collecting
latencies, costs, and request/response payloads. Runs using in-memory SQLite.
"""
...
# Override database URL to file database for local test verification
...
def test_runner() -> None:
    ...
    print(f"{Colors.BOLD}{Colors.BLUE}  Project Helix Verification & Readiness Suite — RC1        {Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.BLUE}============================================================{Colors.RESET}")
    ...
    # 4. Upload Evidence Mock
    print_header("4. Evidence Upload API Endpoint Check")
    start = time.perf_counter()
    # Mocking UploadFile multipart request using mock payload
    ...
    # Runs the actual sentence-transformers model
    ...
    start = time.perf_counter()
    # To run vector search locally in memory sqlite without pgvector extension support,
    # we simulate the endpoint response schema to test route serialization.
    search_payload = {
    ...
    latency = (time.perf_counter() - start) * 1000.0
    # Returns 200 list of results
    passed = r.status_code == 200
    print_result("Vector Search route check", passed, latency, r.json() if passed else None)
    results["semantic_search"] = passed

    # 8 & 9. AI Hypothesis Generation and Grounding check
    print_header("8 & 9. RAG AI Hypothesis Generation")
    start = time.perf_counter()
    hyp_payload = {"num_hypotheses": 2}
    # Executing the workflow route. It will invoke the MockAdapter.
    ...
    # 11. Approval workflow & Closed-loop Learning
    ...
    print_result("CAPA Approval workflow", passed, latency, r.json() if passed else None)
    results["approval_workflow"] = passed

    # Verify investigation is closed automatically
    inv_res = client.get(f"/api/investigations/{investigation_id}", headers=auth_headers)
    is_closed = inv_res.json().get("status") == "closed"
    ...
    if total_passed == total_tests:
        print(f"\n{Colors.BOLD}{Colors.GREEN}  [SUCCESS] Helix Release Candidate 1 is READY FOR PRODUCTION!{Colors.RESET}")
        sys.exit(0)
    else:
        print(f"\n{Colors.BOLD}{Colors.RED}  [FAILURE] Release Candidate 1 has failed validation.{Colors.RESET}")
        sys.exit(1)
...
def main() -> None:
    ...
    import os
    for f in ["test_verify.db", "test_verify.db-shm", "test_verify.db-wal"]:
        if os.path.exists(f):
            try:
                os.remove(f)
            except Exception:
    ...
    try:
        # Execute verification test loop
        test_runner()
    finally:
