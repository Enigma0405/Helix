#!/usr/bin/env python3
"""Project Helix — Neon Cloud Database Integration Verification Script.

Systematically validates backend, database connection, pgvector similarity search,
and workflow logic against the live Neon PostgreSQL cloud database.
"""
...
print(f"Targeting Neon database URL: {db_url}")

# Set overrides for Neon verification
os.environ["DATABASE_URL"] = db_url
os.environ["INFERENCE_PROVIDER"] = get_env_value("INFERENCE_PROVIDER") or "mock"
...
async def setup_db() -> None:
    ...
    async with engine.begin() as conn:
        print("Verifying/creating pgvector extensions...")
        await conn.execute(text("CREATE EXTENSION IF NOT EXISTS vector;"))
        await conn.execute(text("CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";"))
    async with engine.begin() as conn:
        print("Creating table schemas on Neon database...")
...
def test_runner() -> None:
    ...
    print(f"{Colors.BOLD}{Colors.BLUE}  Project Helix Verification Suite — Neon Cloud PostgreSQL  {Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.BLUE}============================================================{Colors.RESET}")

    results = {}
    auth_headers = {}

    # Unique identifier for the test run to avoid unique constraints collisions
    ...
    reg_payload = {
        "email": f"analyst-{test_run_id}@apex.ai",
        "password": "SecurePassword123",
        "full_name": "Jennifer Martinez",
        "org_name": f"Apex Precision Mfg {test_run_id}",
        "org_slug": f"apex-precision-{test_run_id}"
    }
    ...
    start = time.perf_counter()
    inv_payload = {
        "title": f"Batch #2847 — Sterility Failure {test_run_id}",
    ...
    asset_payload = {
        "asset_type": "machine",
        "asset_code": f"FIL-022-A-{test_run_id}",
    ...
    latency = (time.perf_counter() - start) * 1000.0
    passed = text_content == "Filter bubble point integrity test passed for FIL-022-A." and meta["adapter"] == "text"
    print_result("Plain text adapter parser", passed, latency, {"text": text_content[:50], "meta": meta} if passed else None)
    ...
    # 8 & 9. RAG AI Hypothesis Generation
    ...
    print(f"{Colors.BOLD}{Colors.BLUE}  Helix Neon Integration Testing Summary                    {Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.BLUE}============================================================{Colors.RESET}")
    ...
    if total_passed == total_tests:
        print(f"\n{Colors.BOLD}{Colors.GREEN}  [SUCCESS] Helix database works perfectly on Neon PostgreSQL!{Colors.RESET}")
        sys.exit(0)
    else:
        print(f"\n{Colors.BOLD}{Colors.RED}  [FAILURE] Helix Neon validation failed.{Colors.RESET}")
        sys.exit(1)
